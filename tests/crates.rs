...
fn test_create_crate(){
    ...
    let response = client.post(common::CRATES_BASE_URL).json(&request_body).send().unwrap();
...
fn test_update_crate(){
    ...
    let response = client.put(format!("{}/{}", common::CRATES_BASE_URL, a_crate["id"])).json(&update_crate_json).send().unwrap();

    assert_eq!(response.status(), StatusCode::OK);

    let reponse_json :Value = response.json().unwrap();
    assert_eq!(reponse_json, json!({
        "id": a_crate["id"],
        "rustacean_id": a_crate["rustacean_id"],
...
fn get_one_crate(){
    ...
    let response = client.get(format!("{}/{}", common::CRATES_BASE_URL, a_crate["id"])).send().unwrap();
...
fn delete_crate(){
    ...
    let response = client.delete(format!("{}/{}", common::CRATES_BASE_URL, a_crate["id"])).send().unwrap();
